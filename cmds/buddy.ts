...
async function buddy({api, event}) {
  try {
    ...
    const response = await axios.post(apiUrl, { prompt: prompt.join(" "), userName });
    if (response.data && response.data.openai && response.data.openai.generated_text) {
            ...
            api.sendMessage("Error processing the prompt. Please try again later.", event.threadID, event.messageID);
    }
  } catch (err) {
